...
TEST_GROUP(qspi_flash_fast_read_stress_thruput_div6);

TEST_SETUP(qspi_flash_fast_read_stress_thruput_div6)
{
...
TEST_TEAR_DOWN(qspi_flash_fast_read_stress_thruput_div6)
{
    qspi_flash_fast_read_shutdown(ctx);
}

__attribute__((fptrgroup("unity_test_func")))
TEST(qspi_flash_fast_read_stress_thruput_div6, test_qspi_flash_fast_read_data)
{
    uint8_t buf[250000];
    ...
    /* Theoretical max is 33.3 MB/s */
    TEST_ASSERT_FLOAT_WITHIN(SPEED_MAX / 4, SPEED_MAX, thruput);
}

__attribute__((fptrgroup("unity_test_func")))
TEST(qspi_flash_fast_read_stress_thruput_div6, test_qspi_flash_fast_read_data_nibble_swap)
{
    uint8_t buf[250000];
...
TEST_GROUP_RUNNER(qspi_flash_fast_read_stress_thruput_div6) {
    ...
    RUN_TEST_CASE(qspi_flash_fast_read_stress_thruput_div6, test_qspi_flash_fast_read_data_nibble_swap);
}
